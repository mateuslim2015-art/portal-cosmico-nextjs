  ...
  for (const cardData of [...majorArcana, ...remainingMajorArcana]) {
    ...
    await prisma.tarotCard.upsert({
      ...
      create: {
        ...
        arcana: 'major',
        suit: null,
        shortMeaning: cardData.shortMeaning,
        upright: cardData.longMeaning,
        reversed: cardData.longMeaning,
        imageUrl,
      },
    })
  }
    ...
    for (let i = 0; i < numberCards.length; i++) {
      ...
      await prisma.tarotCard.upsert({
        ...
        create: {
          ...
          upright: `Esta carta representa as energias e características específicas do ${number} no contexto do elemento ${suit.element} e naipe ${suit.displayName}.`,
          reversed: `Aspectos bloqueados ou invertidos do ${number} de ${suit.displayName}.`,
        },
      })
    }
    ...
    for (const court of courtCards) {
      ...
      await prisma.tarotCard.upsert({
        ...
        create: {
          ...
          upright: `O ${court} representa aspectos de personalidade e energias específicas relacionadas ao elemento ${suit.element} e naipe ${suit.displayName}.`,
          reversed: `Aspectos bloqueados ou invertidos do ${court} de ${suit.displayName}.`,
        },
      })
    }
  }
